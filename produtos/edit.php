...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['imagem']['name'])) {
        $novoArquivo = processarUploadImagem($_FILES['imagem'], $erros);
        if ($novoArquivo) {
    ...
    if (empty($erros)) {
        if ($nomeArquivo !== $produto['imagem'] && $produto['imagem']) {
            $caminhoAntigo = __DIR__ . '/../uploads/produtos/' . $produto['imagem'];
        ...
        $stmt = $pdo->prepare(
            'UPDATE produtos SET nome = :nome, imagem = :imagem, preco = :preco, localizacao = :localizacao,
...
<div class="card">
    <?php if ($produto['imagem']): ?>
        <div class="current-image">
            <img src="<?= $base ?>uploads/produtos/<?= htmlspecialchars($produto['imagem']) ?>" alt="Imagem atual">
        </div>
    <?php endif; ?>

    <form method="post" action="edit.php?id=<?= (int)$id ?>" enctype="multipart/form-data" novalidate>
        ...
        <div class="form-group">
            ...
            <input type="text" id="localizacao" name="localizacao" value="<?= htmlspecialchars($dados['localizacao'] ?? '') ?>" maxlength="255">
        </div>
